...
from cgmodsel.CG_CLZ_Huber import CG_CLZ_Huber # Huber solver for CLZ models (triple interactions)
...
def get_graph_from_data(filename, model='PW', graphthreshold=1e-2, kS=1, **kwargs):
    ...
    dSolvers = {'PW': None, 'CLZ': CG_CLZ_Huber} # TODO: add solver for sparse pairwise model (e.g., CG_PWS_Huber)
...
#    get_graph_from_data(filename)
    ...
    solver = CG_CLZ_Huber(meta, useweights=True) # initialize problem
    solver.drop_data(D, Y)
    solver.set_regularization_params(kS)
